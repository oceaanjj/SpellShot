...
function spawnLetterHitParticles(x, y) {
  const fx = (window.BallSkins ? window.BallSkins.getHitEffect() : null) || {
    colors: ['#ffe06a'], count: 26,
    speedMin: 2, speedMax: 6, sizeMin: 2, sizeMax: 3,
    decay: 0.018, gravity: 0.08, drag: 0.95,
    shape: 'circle', blendMode: 'lighter', flashColor: 'rgba(255,220,100,',
  };
  ...
  letterHitParticles.forEach(p => {
    ...
    switch (p.shape) {
      ...
      case 'shard':
        ...
        ctx.lineWidth = 0.8;
        ctx.beginPath();
        ctx.moveTo(0, -r * 2.5); ctx.lineTo(0, r * 1.0);
        ctx.stroke();
        break;
...
function drawLetterSuccessFlash() {
  ...
  const grad = ctx.createRadialGradient(
    successFlash.x, successFlash.y, 0,
    successFlash.x, successFlash.y, radius
  );
  ...
  ctx.fillStyle = grad;
  ctx.fillRect(
    successFlash.x - radius, successFlash.y - radius,
    radius * 2, radius * 2
  );
  ctx.restore();
}
