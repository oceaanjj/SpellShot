...
function drawScene(mainCtx, bgImg, ts) {
  ...
  const vignette = mainCtx.createRadialGradient(
    GAME_W / 2, GAME_H / 2, GAME_H * 0.28,  // inner circle (clear)
    GAME_W / 2, GAME_H / 2, GAME_H * 0.85   // outer circle (dark)
  );
  ...
  const drawCharacter = (character, xOffset, alpha = 1) => {
    ...
    mainCtx.drawImage(
      ...
      idleFrame * fw, 0, fw, fh,                        // which frame to cut from the sprite sheet
      0, 0, fw * character.scale, fh * character.scale  // where to draw it on canvas
    );
